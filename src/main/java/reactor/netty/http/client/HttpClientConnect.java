...
final class HttpClientConnect extends HttpClient {
	...
	public Mono<? extends Connection> connect() {
		...
		if (config.deferredConf != null) {
			return Mono.fromCallable(() -> new HttpClientConnect(config))
			           .transform(config.deferredConf)
			           .flatMap(MonoHttpConnect::new);
		}
		else {
			mono = new MonoHttpConnect(this);
		}
	...
	static final class MonoHttpConnect extends Mono<Connection> {
		...
		final HttpClient httpClientConnect;

		MonoHttpConnect(HttpClient httpClientConnect) {
			this.httpClientConnect = httpClientConnect;
		}

		@Override
		public void subscribe(CoreSubscriber<? super Connection> actual) {
			HttpClientConfig config = httpClientConnect.configuration();
			...
			Mono.<Connection>create(sink -> {
				...
				AddressResolverGroup<?> resolver = _config._resolver();
