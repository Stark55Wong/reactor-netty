...
public final class HttpClientConfig extends TransportClientConfig<HttpClientConfig> {
	...
	Function<Mono<HttpClientConnect>, Mono<HttpClientConnect>> deferredConf;
	...
	/**
	 * Return the configure base URL to use for this request/response.
	 *
	 * @return the configure base URL to use for this request/response
	 */
	...
	/**
	 * Return the configured I/O handler for the request.
	 *
	 * @return the configured I/O handler for the request
	 */
	public BiFunction<? super HttpClientRequest, ? super NettyOutbound, ? extends Publisher<Void>> body() {
		return body;
	}

	@Override
	public int channelHash() {
		return Objects.hash(super.channelHash(), acceptGzip, sslProvider);
	}
	...
	public Function<Mono<HttpClientConnect>, Mono<HttpClientConnect>> deferredConf() {
		return deferredConf;
	}
	 ...
	 * @return
	 */
	public boolean isWebsocketProxyPing() {
		return websocketProxyPing;
	}
	 ...
	 * @return the configured maximum allowable frame payload length
	 */
	public int websocketMaxFramePayloadLength() {
		return websocketMaxFramePayloadLength;
	}

	/**
	 * Return the configured subprotocol comma separated list.
	 *
	 * @return the configured subprotocol comma separated list
	 */
		...
		return super.defaultOnChannelInit()
		            .then((channel, observer, remoteAddress) -> {
		                ...
		                if ((protocols & h11) == h11) {
		                    configureHttp11Pipeline(channel.pipeline(), acceptGzip, decoder.initialBufferSize(), decoder.maxChunkSize(),
		                    decoder.failOnMissingResponse, decoder.maxHeaderSize(), decoder.maxInitialLineLength(),
		                    metricsRecorder(), decoder.parseHttpAfterConnectRequest, decoder.validateHeaders());
		                }
		            });
	}
	...
	void deferredConf(Function<HttpClientConnect, Mono<HttpClientConnect>> deferrer) {
