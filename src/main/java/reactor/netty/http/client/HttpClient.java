...
public abstract class HttpClient extends TransportClient<HttpClient, HttpClientConfig> {
	...
	public final HttpClient compress(boolean compressionEnabled) {
		...
		if (compressionEnabled) {
			dup.configuration().headers.add(HttpHeaderNames.ACCEPT_ENCODING, HttpHeaderValues.GZIP);
			dup.configuration().acceptGzip = true;
		}
		else {
			if (isCompressing(dup.configuration().headers)) {
				dup.configuration().headers.remove(HttpHeaderNames.ACCEPT_ENCODING);
			}
	...
	public final HttpClient cookie(Cookie cookie) {
		...
		if (!cookie.value().isEmpty()) {
			HttpClient dup = duplicate();
			dup.configuration().headers.add(HttpHeaderNames.COOKIE, dup.configuration().cookieEncoder.encode(cookie));
			return dup;
		}
		return this;
	}
	...
	public final HttpClient cookiesWhen(String name, Function<? super Cookie, Mono<? extends Cookie>> cookieBuilder) {
		...
		Objects.requireNonNull(cookieBuilder, "cookieBuilder");
		HttpClient dup = duplicate();
		dup.configuration().deferredConf(client ->
				cookieBuilder.apply(new DefaultCookie(name, ""))
				             .map(c -> {
				                 if (!c.value().isEmpty()) {
				                     client.configuration().headers.add(HttpHeaderNames.COOKIE,
				                         client.configuration().cookieEncoder.encode(c));
				                 }
				                 return client;
				             }));
		return dup;
	...
	public final HttpClient headers(Consumer<? super HttpHeaders> headerBuilder) {
		...
		HttpClient dup = duplicate();
		headerBuilder.accept(dup.configuration().headers);
		return dup;
	}
	...
	public final HttpClient headersWhen(Function<? super HttpHeaders, Mono<? extends HttpHeaders>> headerBuilder) {
		...
		HttpClient dup = duplicate();
		dup.configuration().deferredConf(client ->
				headerBuilder.apply(client.configuration().headers)
				             .map(h -> {
				                 client.configuration().headers = h;
				                 return client;
				             }));
		return dup;
	}
	...
	public final HttpClient httpResponseDecoder(Function<HttpResponseDecoderSpec, HttpResponseDecoderSpec> responseDecoderOptions) {
		...
		HttpClient dup = duplicate();
		dup.configuration().decoder = responseDecoderOptions.apply(new HttpResponseDecoderSpec()).build();
		return dup;
	}
	...
	public final HttpClient keepAlive(boolean keepAlive) {
		HttpClient dup = duplicate();
		HttpUtil.setKeepAlive(dup.configuration().headers, HttpVersion.HTTP_1_1, keepAlive);
		return dup;
	}
	...
	public final HttpClient secure() {
		HttpClient dup = duplicate();
		dup.configuration().sslProvider = HttpClientSecure.sslProvider(null);
		return dup;
	}
	...
	public final HttpClient secure(Consumer<? super SslProvider.SslContextSpec> sslProviderBuilder) {
		...
		dup.configuration().sslProvider = HttpClientSecure.sslProvider(((SslProvider.Builder) builder).build());
		return dup;
	}
	...
	public final WebsocketSender websocket(String subprotocols, int maxFramePayloadLength, boolean proxyPing) {
		...
		config.websocketMaxFramePayloadLength = maxFramePayloadLength;
		config.websocketProxyPing = proxyPing;
		config.websocketSubprotocols = subprotocols;
